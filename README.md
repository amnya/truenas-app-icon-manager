...
**TrueNAS App Icon Manager** is a self-hosted web application for managing custom app icons and Web UI buttons in the **TrueNAS SCALE Apps UI**.

It is built for **TrueNAS SCALE 25.10.1** and the newer **Docker-based Apps** system. It helps users persist and reapply custom icons and custom app Web UI URLs by safely patching the generated Apps metadata YAML file.
...
For custom apps that do not show a TrueNAS Web UI button, the app can also manage a single `Web UI` portal URL:

```yaml
<app-name>:
  portals:
    Web UI: "http://<truenas-host>:<port>"
```
...
- Shows detected host port badges when TrueNAS metadata exposes app port mappings
- Filters for all apps, missing icons only, and custom apps only
- Adds or updates a `Web UI` button URL for custom apps
...
- Patches `<app-name>.portals.Web UI` in TrueNAS generated metadata YAML for managed custom app Web UI buttons
...
That file is managed by TrueNAS. TrueNAS may overwrite it during redeploys, app edits, app updates, or system reboot. This app is designed around that behavior: it keeps your desired icon and Web UI mappings in `/config/icon-mappings.json` and reapplies them when needed.
...
```text
ghcr.io/amnya/truenas-app-icon-manager:v1.1.0
```
...
### Add A Web UI Button For A Custom App

Custom apps created directly in TrueNAS may not show a Web UI button. For custom apps, open the app card, enter the URL in **Web UI button**, and click Save.

If TrueNAS metadata exposes port mappings for the app, the dashboard shows small port badges such as `Port TCP 18080->8080`. Use the host-side port when building the Web UI URL.

The app stores that URL in `/config/icon-mappings.json` and patches:

```text
<app-name>.portals.Web UI
```

Only absolute `http://` and `https://` URLs are accepted. The label is fixed as `Web UI` so TrueNAS shows one predictable button.
...
- `POST /api/portals/:appName`
- `DELETE /api/portals/:appName`
- `POST /api/reapply`
- `GET /api/logs`
- `GET /api/status`
- `GET /api/health`
...
### Save A Custom App Web UI URL

```bash
curl -X POST http://<truenas-host>:8099/api/portals/alphaedge \
  -H "Content-Type: application/json" \
  -d '{"url":"http://<truenas-host>:<app-port>/"}'
```

### Remove A Managed Web UI URL

```bash
curl -X DELETE http://<truenas-host>:8099/api/portals/alphaedge
```
...
- Preserve unrelated portal fields
...
- Multiple reapply runs do not duplicate Web UI portal keys
...
- Docker image publishing to GitHub Container Registry when a tag like `v1.1.0` is pushed
